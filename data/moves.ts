...
export const Moves: { [moveid: string]: MoveData } = {
	...
	overbite: {
		...
		type: "Dark",
		onAfterHit(target, source) {
			this.damage(Math.round(source.maxhp / 2), source, source, this.dex.getEffect('Overbite'), true);
		},
		flags: {contact: 1, protect: 1, mirror: 1},
		isNonstandard: "Future",
	},
	...
	mop: {
		...
		onHit(pokemon) {
			...
			for (const condition of sideConditions) {
				if (pokemon.hp && pokemon.side.removeSideCondition(condition)) {
					this.add('-sideend', pokemon.side, this.dex.getEffect(condition).name, '[from] move: Mop', '[of] ' + pokemon);
	...
	stonehenge: {
		...
		onAfterHit(target, pokemon) {
			...
			for (const condition of sideConditions) {
				if (pokemon.hp && pokemon.side.removeSideCondition(condition)) {
					this.add('-sideend', pokemon.side, this.dex.getEffect(condition).name, '[from] move: Stonehenge', '[of] ' + pokemon);
				}
			}
			if (pokemon.hp && pokemon.volatiles['partiallytrapped']) {
				pokemon.removeVolatile('partiallytrapped');
